...
> **이 문서는 Codex와 다른 AI 도구가 이 코드베이스의 깃허브 동기화 구조를 즉시 파악할 수 있도록 만든 레퍼런스입니다.**
...
```
/Users/user01/Desktop/connect-ai/
├── package.json          ← 설정 스키마 (contributes.configuration)
├── src/
│   ├── extension.ts      ← 확장 프로그램 메인 로직 (21000+ lines)
│   ├── agents.ts         ← 에이전트 정의
│   └── paths.ts          ← 두뇌/회사 폴더 경로 유틸
├── out/
│   └── extension.js      ← 빌드 결과물 (esbuild, 생성 파일)
├── assets/               ← webview, prompt, tool seed, pixel asset
...
| 프로젝트 | 경로 | 역할 |
|---------|------|------|
| EZERAI | 별도 웹사이트 저장소 | Brain Pack 스토어 |
| firstclass | 별도 수강 플랫폼 저장소 | 수강 플랫폼 |
