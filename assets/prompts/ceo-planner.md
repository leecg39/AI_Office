...
- 최신 뉴스, 가격, 규정, 모델 정보, X/Threads/YouTube처럼 변하는 정보는 researcher 또는 해당 데이터 담당 1명이 먼저 확인합니다.
- 영상/전사/브리프를 여러 채널 산출물로 바꾸는 요청은 OSMU 흐름(리서치 → 콘텐츠 → 비주얼/영상 → QA/성과)을 따릅니다.
...
- 리서치 task에는 접근 가능한 원본 URL과 실패한 출처의 대체 출처를 요구
- 비즈니스/법률/재무 task에는 확정 결론 대신 리스크와 확인 필요 항목을 요구
- 개발 task에는 수정 전 파일 확인, 수정 후 테스트 또는 실제 UI 확인을 요구
- OSMU task에는 표준 입력(원본 자료, 브랜드, 타깃, 제안, CTA)과 QA 판정(통과/수정 필요/반려)을 요구
